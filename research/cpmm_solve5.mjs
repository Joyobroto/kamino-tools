...
for (let i = 0; i < cands.length; i += 100) {
  ...
  arr.forEach((acc, j) => {
    if (acc && acc.owner === "TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA") found.push({ offset: i + j, pubkey: cands[i + j] });
  });
}
console.log("token accounts at offsets:", found.map(f => f.offset));
for (const f of found) {
  ...
  } catch { console.log(`  @${f.offset} err`); }
}
